...
Each individual checkbox item requires both an input and a label, and are linked together with matching `id` and `for` attributes. All `name` attributes within a checkbox group are also required to be the same for each input, and must be unique for each checkbox group used in a form. The form description is optional.
...
Add information about select lists.
...
Add information about file uploads.
